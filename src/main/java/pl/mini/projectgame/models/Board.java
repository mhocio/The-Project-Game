...
public class Board {
    ...
    public synchronized void movePlayer(BoardObject object, Position source, Position target)
            throws DeniedMoveException {

        if(!cells.get(source).getContent().equals(object)) {
        ...
        cells.get(target).setContent(object);
