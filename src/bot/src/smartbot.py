...
class Player:
    ...
    def init_config(self):
        ...
        if config["action"] == "startGame":
                self.set_board(config["board"]["width"], config["board"]["taskAreaHeight"] + 2 * config["board"]["goalAreaHeight"], config["board"]["goalAreaHeight"])
    ...
    def discoverAndTryToPickUpAll(self):
        ...
        if((abs(self.get_pos_x()-px)+abs(self.get_pos_y()-py))<2):
            ...
            for i in range(0,minVal):
                if(self.get_pos_x+(self.get_pos_x-px)*i<0 or self.get_pos_x()+(self.get_pos_x()-px)*i > self.board.board_width 
                or self.get_pos_y+(self.get_pos_y-py)*(minVal-i)< 0 or self.get_pos_y+(self.get_pos_y-py)*(minVal-i) > self.board.board_height):
                    continue
